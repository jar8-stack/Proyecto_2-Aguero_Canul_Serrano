...
import com.example.proyecto_1_aguero_castillo_canul_serrano.db.User
...
class SelectPlayerActivity : AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        if(misPreferencias.getLogeado()){
            ...
            var dataUsuarios = db.userDao().getUsers()

            val convert_from_spinner: Spinner = findViewById(R.id.spinnerUsuarios)
            convert_from_spinner.adapter = ArrayAdapter<User>(this, android.R.layout.simple_list_item_1, dataUsuarios)

            fabAgregarUsuario.setOnClickListener{ v ->
                var dialog: AlertDialog? = null
                val builder = AlertDialog.Builder(this)
                // set the custom layout
                val view = layoutInflater.inflate(R.layout.layout_new_user, null)

                val nombreUsuario: EditText = view.findViewById(R.id.edtNombreUsuario) // Get reference of your XML views

                val buttonAgregarUsuario: Button = view.findViewById(R.id.bt_done);
                buttonAgregarUsuario.setOnClickListener{ v ->
                    ...
                    if(nombreUsuario.text.toString() == ""){
                        ...
                        db.configurationDao().insertConfiguration(5,"M",3,true);
                        ...
                        convert_from_spinner.adapter = ArrayAdapter<User>(this, android.R.layout.simple_list_item_1, nuevaData)
                        ...
                        Toast.makeText(
                            this,
                            nombreUsuario.text.toString() + " agregado",
                            Toast.LENGTH_SHORT
                        ).show()
            ...
            val btnSeleccionarDeUsuario: Button = findViewById(R.id.btnSeleccionarUsuario);
            btnSeleccionarDeUsuario.setOnClickListener{ v ->
                val text = convert_from_spinner.getSelectedItem().toString();
                val parts = text.split("-")

                misPreferencias.setLogeado(true)
                misPreferencias.setNombreUsuario(parts[0])
                misPreferencias.setIdUsuario(parts[1]);
